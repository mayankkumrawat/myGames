...
function moveAsteroid() {
    ...
    asteroids.forEach(element => {
        ...
        if (element.y >= 580) {
            ...
            element.style.left = Math.round((gameSpaceRect.width - 90) * Math.random()) + 'px';
        }
...
function gamePlay() {
    ...
    if (player.start) {
        ...
        if (keys.ArrowRight && (player.x + 50 + 23) < (gameSpaceRect.width)) {
            player.x += player.speed;
        }
        if (keys.ArrowUp) {
            fireButton.click();
...
function start() {
    ...
    for (i = 0; i < 4; i++) {
        ...
        asteroid.style.left = Math.round(Math.random() * (gameSpaceRect.width - 90)) + 'px';
